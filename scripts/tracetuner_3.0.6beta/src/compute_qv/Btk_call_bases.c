...
/*
 *  Btk_call_bases.c $Revision: 1.25.2.3 $
 */
 ...
 */
static int
check_spacing(int data_peak_ind, Data *data, double expect_spacing, Options *op)
{
   ...
   if (expect_spacing <= 0 && op->Verbose > 1) {
       fprintf(stderr, 
    ...
    Options *options)
{
           ...
           (j-base_index < 3))
    {
            ...
            (pk->iheight > pkp1->iheight*FRAC_SHORT_CALLED_PEAK))
        {
            ...
            spacing_problems = check_spacing(i, data, curr_spacing, options);
            j++;
            continue;
        }
    ...
    if (INSERT_BASES)
    {
        ...
        for (i = 0; i < data->peak_list_len; i++) 
        {
            ...
            if (peak_outside_data_range(data->peak_list[i], data))
             ...
             if (data->peak_list[i]->is_called > 0) {
                 detect_spacing_problems = check_spacing(i, data, curr_spacing, options); 
             }
