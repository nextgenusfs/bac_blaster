...
{
    Stats stats;
    stats.min_ = stats.max_ = stats.count_ = stats.mean_ 
               = stats.var_ = -1.;
    return stats;
}
...
{
    Histo histo;
    histo.x_min_ = histo.x_max_ = histo.bin_to_x_ = histo.x_to_bin_ = -1.;
    return histo;
}
...
mobShiftQuality(Peak * const peak[], int n_peaks)
{
    ...
    const Peak *pk0, *pk1;
    Histo       spac_histo_old = initialize_histo(), 
                spac_histo_new = initialize_histo();
                    ...
                    Results *results )
{
    int i, chunk_num = 0;
    Peak *pk0, *pk1;
    Stats stats = initialize_stats();
    Histo histo = initialize_histo();
    Peak  **dip;
